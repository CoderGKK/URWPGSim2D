...
namespace URWPGSim2D.Match
{
    ...
    /// <summary>
    /// 对抗比赛3V3仿真使命特有的仿真机器鱼类
    /// </summary>
    [Serializable]
    public class Fish3V3 : RoboFish
    {
        /// <summary>
        /// 记录仿真机器鱼在己方禁区的次数
        /// </summary>
    ...
    /// </summary>
    [Serializable]
    public class Environment3V3 : SimEnvironment
    {
        // 在这里定义对抗比赛3V3仿真使命特有的仿真环境的特性和行为
    }
    ...
    /// </summary>
    [Serializable]
    public partial class Match3V3 : Mission
    {
        #region Singleton设计模式实现让该类最多只有一个实例且能全局访问
        private static Match3V3 instance = null;
        ...
        public static Match3V3 Instance()
        {
            if (instance == null)
            {
                instance = new Match3V3();
            }
            return instance;
        }
        ...
        /// </summary>
        public List<Team<Fish3V3>> Teams = new List<Team<Fish3V3>>();

        /// <summary>
        /// 仿真使命（比赛或实验项目）所使用的具体仿真环境
        /// </summary>
        public Environment3V3 Env = new Environment3V3();
        #endregion
        ...
        private void InitTeamsAndFishes()
        {
            for (int i = 0; i < CommonPara.TeamCount; i++)
            {
                ...
                for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
                {
                    ...
                    Teams[i].Fishes.Add(new Fish3V3());
        ...
        private void DeadBallHandler()
        {
            if (IsDeadBall())
            {
                ...
                Team<Fish3V3> teamLeft = (CommonPara.IsExchangedHalfCourt == true) ? Teams[1] : Teams[0];
                // 交换半场前/后右半场的队伍分别为第1/0支队伍
                Team<Fish3V3> teamRight = (CommonPara.IsExchangedHalfCourt == true) ? Teams[0] : Teams[1];
                for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
        ...
        #region 犯规情况1判断及处理
        // Add By ChenPenghui 2010-04-01 Modified by renjing 20110310
        /// <summary>
        /// 处理犯规情况1：仿真水球在己方禁区时，己方仿真机器鱼超过两条在己方禁区内，并且持续时间超过一定周期数
        /// </summary>
        private void FoulType1Handler()
        {
            ...
            {// 仿真水球位于左边禁区
                ...
                Team<Fish3V3> team = (CommonPara.IsExchangedHalfCourt == true) ? Teams[1] : Teams[0];
            ...
            {// 仿真水球位于右边禁区
                ...
                Team<Fish3V3> team = (CommonPara.IsExchangedHalfCourt == true) ? Teams[0] : Teams[1];
            ...
            {// 犯规处理
                ...
                {// 使用countFishFouled记录单支队伍罚下仿真机器鱼的条数用于确定罚下场后的摆放位置
                    Fish3V3 fish = Teams[i].Fishes[j];
                    if (fish.FoulFlag == FoulType.FOUl1)
                    {
                        countFishFouled++;
        ...
        private void FoulType1Handler(ref Team<Fish3V3> team, bool bIsLeftHalfCourt)
        {
            // 犯规的仿真机器鱼在队伍中的索引
            ...
            {//允许两条鱼在己方禁区内的代码，有犯规1情况出现时的处理
                ...
                team.Fishes[theLastFishEnterIndex].FoulFlag = FoulType.FOUl1;
        ...
        private void FoulType2Handler(ref Team<Fish3V3> team, bool bIsLeftHalfCourt)
        {
            // 犯规的仿真机器鱼在队伍中的索引
            List<int> arrFishIndexFouled = new List<int>();
        ...
        /// <param name="team">待处理的仿真使命参与队伍（仿真机器鱼为具体使命的仿真机器鱼）</param>
        /// <param name="bIsLeftHalfCourt">待判断的是否左球门true判断左球门false判断右球门</param>
        private void GoalHandler(ref Team<Fish3V3> team, bool bIsLeftHalfCourt)
        {
            MatchHelper.MissionMessageBox(ref MyMission.Instance().MissionRef,
