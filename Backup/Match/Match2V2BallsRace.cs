...
namespace URWPGSim2D.Match
{
    ...
    public partial class Match2V2BallRace : Mission
    {
        ...
        public void ResetDynamicObstacles()
        {
            ...
            int r = Env.Balls[0].RadiusMm;
            // 将2个仿真方形障碍物恢复默认位置
            Env.DynamicRect[0].PositionMm = new xna.Vector3(-1200 + Env.DynamicRect[0].LengthMm / 2, 0, 300);
            Env.DynamicRect[1].PositionMm = new xna.Vector3(1200 - Env.DynamicRect[1].LengthMm / 2, 0, -300);

            // 提供从界面修改仿真场地尺寸及障碍物各项参数的功能后恢复默认就不仅仅是恢复默认位置了
            for (int i = 0; i < 2; i++)
            {// 所有仿真方形障碍物长度/宽度相同
                Env.DynamicRect[i].IsDeletionAllowed = false; // 不允许删除
                Env.DynamicRect[i].LengthMm = 6 * r;
                //Env.ObstaclesRect[i].LengthMm = r;
