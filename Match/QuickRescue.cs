...
// Date: 20180516  Author: HuZhe Version: 1
// Description: 急速救援相关信息
//-----------------------------------------------------------------------
...
namespace URWPGSim2D.Match
{
    ...
    /// </summary>
    [Serializable]
    public class FishRescue : RoboFish
    {
        /// <summary>
        /// 记录仿真机器鱼在己方禁区的次数
        /// </summary>
    ...
    /// </summary>
    [Serializable]
    public class EnvironmentRescue : SimEnvironment
    {
    ...
    /// </summary>
    [Serializable]
    public partial class MatchRescue : Mission
    {
        #region Singleton设计模式实现让该类最多只有一个实例且能全局访问
        private static MatchRescue instance = null;
        ...
        public static MatchRescue Instance()
        {
            if (instance == null)
            {
                instance = new MatchRescue();
            }
            return instance;
        }
        ...
        /// </summary>
        public List<Team<FishRescue>> Teams = new List<Team<FishRescue>>();

        /// <summary>
        /// 仿真使命（比赛或实验项目）所使用的具体仿真环境
        /// </summary>
        public EnvironmentRescue Env = new EnvironmentRescue();
        #endregion
        ...
        private void InitTeamsAndFishes()
        {
            for (int i = 0; i < CommonPara.TeamCount; i++)
            {
                ...
                Teams[i].Fishes.Add(new FishRescue());
        ...
        private void InitEnvironment()
        {
            ...
            xna.Vector3 positionMm1 = new xna.Vector3(550, 0, 950);
            xna.Vector3 positionMm2 = new xna.Vector3(-550, 0,950);
;
            xna.Vector3 positionMm3 = new xna.Vector3(550, 0,-950);
            xna.Vector3 positionMm4 = new xna.Vector3(-550, 0, -950);
            Env.ObstaclesRect.Add(new RectangularObstacle("obs1", positionMm, Color.Green, 100, 1100, 0));
            Env.ObstaclesRect.Add(new RectangularObstacle("obs2", positionMm1, Color.Green, 100,1100, 0));
            Env.ObstaclesRect.Add(new RectangularObstacle("obs3", positionMm2, Color.Green, 100, 1100, 0));
            Env.ObstaclesRect.Add(new RectangularObstacle("obs4", positionMm3, Color.Green, 100, 1100, 0));
            Env.ObstaclesRect.Add(new RectangularObstacle("obs5", positionMm4, Color.Green, 100, 1100, 0));
       
        }

        private int[] gool = { 0,0 };
        private int[] goor = { 0, 0 };
        ...
        public override void ResetTeamsAndFishes()
        {
            ...
            if (Teams != null)
            {
             ...
             //   if (CommonPara.IsExchangedHalfCourt == false)
                {
                    // 左半场队伍的仿真机器鱼位置
                    //myMission.TeamsRef[0].Fishes[0].PositionMm = new xna.Vector3(                        f.LeftMm + myMission.TeamsRef[0].Fishes[0].BodyLength * 3, 0, 0);
                    ...
                    myMission.TeamsRef[0].Fishes[0].PositionMm = new xna.Vector3(
                        1350, 0, 700);
                    myMission.TeamsRef[0].Fishes[1].PositionMm = new xna.Vector3(1350, 0, -700);

                    for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
                    {
                        // 左半场队伍的仿真机器鱼朝右
                        myMission.TeamsRef[1].Fishes[j].BodyDirectionRad = 0;
                        // 右半场队伍的仿真机器鱼朝左
                        myMission.TeamsRef[0].Fishes[j].BodyDirectionRad = xna.MathHelper.Pi;
                    }
                }
        ...
        public override void ResetObstacles()
        {
            ...
            Env.Balls[0].PositionMm = new xna.Vector3(-1350, 0, 1050);
            Env.Balls[1].PositionMm = new xna.Vector3(-1350, 0, -1050);
            Field f = Field.Instance();
        ...
        private int[] ltime={0,0,0};
        private int[] rtime={0,0,0};
        ...
        private void GameOverHandler()
        {
            ...
            {
                bool flag = false;
                for (int jj = 2; jj >= 0 && !flag; jj--)
                { 
                CommonPara.IsRunning = false;
                if (ltime[jj] < rtime[jj])
                {
                    flag = true;
                    int winner = 0;
                    strMsg = string.Format("Congradulations!\nCompetition is completed.\nWinner:【{0}】 in 【{1}】\n",
