...
// Description: 两队对抗赛1VS1相关的仿真机器鱼，仿真环境和仿真使命定义文件
// Histroy:
...
{
    /// <summary>
    /// 对抗比赛1V1仿真使命特有的仿真机器鱼类
    /// </summary>
    [Serializable]
    public class Fish1V1 : RoboFish
    {
        /// <summary>
        /// 记录仿真机器鱼在己方球门区的次数
        /// </summary>
    ...
    /// </summary>
    [Serializable]
    public class Environment1V1 : SimEnvironment
    {
        // 在这里定义对抗比赛1V1仿真使命特有的仿真环境的特性和行为
    }
    ...
    /// </summary>
    [Serializable]
    public partial class Match1V1 : Mission
    {
        #region Singleton设计模式实现让该类最多只有一个实例且能全局访问
        private static Match1V1 instance = null;
        ...
        public static Match1V1 Instance()
        {
            if (instance == null)
            {
                instance = new Match1V1();
            }
            return instance;
        }
        ...
        /// </summary>
        public List<Team<Fish1V1>> Teams = new List<Team<Fish1V1>>();

        /// <summary>
        /// 仿真使命（比赛或实验项目）所使用的具体仿真环境
        /// </summary>
        public Environment1V1 Env = new Environment1V1();  
        #endregion
        ...
        private void InitTeamsAndFishes()
        {
            for (int i = 0; i < CommonPara.TeamCount; i++)
            {
                ...
                for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
                {
                    ...
                    Teams[i].Fishes.Add(new Fish1V1());
            ...
            if (Teams != null)
            {
                for (int i = 0; i < Teams.Count; i++)
                {
                    ...
                    for (int j = 0; j < Teams[i].Fishes.Count; j++)
                    {
                        ...
                        Teams[i].Fishes[j].FoulFlag = FoulType.NONE;
        ...
        private void DeadBallHandler()
        {
            if (IsDeadBall())
            {
                ...
                Team<Fish1V1> teamLeft = (CommonPara.IsExchangedHalfCourt == true) ? Teams[1] : Teams[0];
                // 交换半场前/后右半场的队伍分别为第1/0支队伍
                Team<Fish1V1> teamRight = (CommonPara.IsExchangedHalfCourt == true) ? Teams[0] : Teams[1];
                for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
        ...
        private void FoulType2Handler()
        {
            ...
            {// 仿真水球位于左边禁区
                ...
                Team<Fish1V1> team = (CommonPara.IsExchangedHalfCourt == true) ? Teams[1] : Teams[0];
            ...
            {// 仿真水球位于右边禁区
                ...
                Team<Fish1V1> team = (CommonPara.IsExchangedHalfCourt == true) ? Teams[0] : Teams[1];
            ...
            {// 仿真水球不在禁区 
                for (int i = 0; i < CommonPara.TeamCount; i++)
                {
                    for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
                    {
                        if (Teams[i].Fishes[j].FoulFlag == FoulType.NONE)
            ...
            for (int i = 0; i < CommonPara.TeamCount; i++)
            {// 犯规处理
                for (int countFishFouled = 0, j = 0; j < CommonPara.FishCntPerTeam; j++)
                {// 使用countFishFouled记录单支队伍罚下仿真机器鱼的条数用于确定罚下场后的摆放位置
                    Fish1V1 fish = Teams[i].Fishes[j];
                    if (fish.FoulFlag == FoulType.FOUl2)
                    {
                        countFishFouled++;
        ...
        private void FoulType2Handler(ref Team<Fish1V1> team, bool bIsLeftHalfCourt)
        {
            // 犯规的仿真机器鱼在队伍中的索引
            ...
            {// 犯规情况2提示
                ...
                {
                    team.Fishes[arrFishIndexFouled[i]].FoulFlag = FoulType.FOUl2;
        ...
        private void GoalHandler()
        {
            ...
            else if (IsBallInGoal(Env.Balls[0], false))
            {// 仿真水球进入右边球门
        ...
        /// <param name="team">待处理的仿真使命参与队伍（仿真机器鱼为具体使命的仿真机器鱼）</param>
        /// <param name="bIsLeftHalfCourt">待判断的是否左球门true判断左球门false判断右球门</param>
        private void GoalHandler(ref Team<Fish1V1> team, bool bIsLeftHalfCourt)
        {
            MatchHelper.MissionMessageBox(ref MyMission.Instance().MissionRef,
            ...
            {// 解除犯规状态
                for (int j = 0; j < CommonPara.FishCntPerTeam; j++)
                {
                    team.Fishes[j].FoulFlag = FoulType.NONE;
        ...
        private void HandleBeforePenaltyKick(int teamId)
        {
            ...
            if (teamId % 2 == 0)
            {
                Env.Balls[0].PositionMm = new xna.Vector3((Env.FieldInfo.RightMm-Env.FieldInfo.GoalDepthMm - Env.FieldInfo.ForbiddenZoneLengthXMm), 0, 0);
                Teams[0].Fishes[0].PositionMm = new xna.Vector3(Env.Balls[0].PositionMm.X - Teams[0].Fishes[0].BodyLength * (float)1.1, 0, 0);
                Teams[1].Fishes[0].PositionMm = new xna.Vector3(Env.FieldInfo.RightMm - Teams[1].Fishes[0].BodyLength * (float)1.8, 0, 0);
            }
            //B队
            else
            {
                Env.Balls[0].PositionMm = new xna.Vector3((Env.FieldInfo.LeftMm +Env.FieldInfo.GoalDepthMm+ Env.FieldInfo.ForbiddenZoneLengthXMm), 0, 0);
                Teams[0].Fishes[0].PositionMm = new xna.Vector3(Env.FieldInfo.LeftMm + Teams[0].Fishes[0].BodyLength * (float)1.8, 0, 0);
                Teams[1].Fishes[0].PositionMm = new xna.Vector3(Env.Balls[0].PositionMm.X + Teams[1].Fishes[0].BodyLength * (float)1.1, 0, 0);
            }
        }
